      ...
      if (req.file) {
        ...
        if (!flag) {
          ...
          try {
            ...
            */
            let { file: old_filename, applier_roll } = wrapper(
              row,
              "certificate"
            );
            let final_dest =
...
admin.post("/decline", multer().none(), async function (req, res) {
  ...
  if (flag) {
    try {
      ...
      console.log(err);
      res.status(500).json({
        message: "There was some error declining the file. Try again later",
      });
    }
  }
});
...
admin.get("/", async function (req, res) {
  ...
  try {
    ...
    path_details.forEach(function (ele) {
      ...
      let { path_no, certificate_id, status } = wrapper(
        ele,
        "certificate-path"
      );
      path_objects.push({
    ...
    res
      .status(500)
      .send({ message: "Some issue with the server. Try again later" });
  }
});
...
admin.post("/postal_status", async function (req, res) {
  ...
  try {
    ...
    console.log(err);
    res.status(500).json({
      message: "There was some error uploading the message. Try again later",
    });
  }
});
  ...
  upload(req, res, async function (err) {
    ...
    if (!req.file) {
      ...
      if (req.file) {
        ...
        try {
          let { email_address: to_email, applier_roll } = wrapper(
            id_exists,
            "certificate"
          );
          ...
          let { name: type_name } = wrapper(type_name_row, "certificate-type");
          let mailDetails = {
          ...
          helpers.mailTransporter.sendMail(mailDetails, async function (
            err,
            data
          ) {
            if (err) {
              ...
              res
                .status(500)
                .json({ message: "Unable to send mail. Try again later" });
              fs.unlinkSync(initial_dest);
            } else {
