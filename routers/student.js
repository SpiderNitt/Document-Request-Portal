    ...
    if (err) {
      ...
      try {
        ...
        for (const sem_mapping of rank_grade_mapping) {
          ...
          await database.RankGradeCard.create({
            certificate_id,
            applier_roll,
  ...
  try {
    ...
    if (rows == null) {
      ...
      rows.forEach(function (ele) {
        ...
        if (address != null) response_json.push(address);
      });
      res.status(200).json(response_json);
      return;
    }
...
student.post("/add_certificate", async function (req, res) {
  try {
    ...
    if (type_exists != null) {
      return helper.responseHandle(
        400,
        responseMessages.CERTIFICATE_TYPE_EXIST,
        res
      );
    } else {
      await database.CertificateType.create({
        name: cert_type,
        created_by: username,
        semwise_mapping: req.body.semwise_mapping ? 1 : 0,
      });
      return helper.responseHandle(
        200,
        responseMessages.CREATE_CERTIFICATE,
        res
      );
    }
  } catch (err) {
...
student.get("/certificate_history", async function (req, res) {
  try {
    ...
    if (id) {
      ...
      rows.forEach(function (ele) {
        const { status, path_email, updatedAt, comments } = helper.wrapper(ele);
        response_json.push({
          status,
          path_email,
          time: updatedAt,
