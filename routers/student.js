...
student.get("/certificate_history", async function (req, res) {
    try {
        ...
        if (id) {
            ...
            let { applier_roll, comments } = helpers.wrapper(id_exists);
            let { time } = helpers.wrapper(row);
            let response_json = [];
            response_json.push({
            ...
            rows.forEach(function (ele) {
                const { status, path_email, updatedAt, comments } = helpers.wrapper(
                    ele
                );
                response_json.push({
                    status,
...
student.get("/certificate_types", async function (req, res) {
    try {
        ...
        let response_json = [];
        rows.forEach(function (ele) {
            const { id, name } = helpers.wrapper(ele);
            response_json.push({
                id,
                type: name,
            });
