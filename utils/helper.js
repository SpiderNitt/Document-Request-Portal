...
async function approve_decline_rights(req, res, certificate_id) {
    ...
    if (!flag) {
        res.status(401).json({
            message:
                "You do not have appropriate permissions to approve or reject this certificate",
        });
        return false;
    }
    ...
    if (!flag) {
        res
            .status(400)
            .json({ message: "Faculty before you has not approved/rejected." });
        return false;
    }
    path_array.forEach(function (ele) {
        if (ele.no > current_verified && !ele.status.includes("PENDING")) {
            res
                .status(400)
                .json({ message: "Faculty after you have already approved/declined." });
            return false;
        }
    });
    return true;
